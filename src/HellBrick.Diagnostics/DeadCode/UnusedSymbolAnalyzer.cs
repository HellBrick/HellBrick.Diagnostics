...
namespace HellBrick.Diagnostics.DeadCode
{
	...
	public class UnusedSymbolAnalyzer : DiagnosticAnalyzer
	{
		...
		private void StartAnalysis( CompilationStartAnalysisContext context )
		{
			...
			context.RegisterSyntaxTreeAction( treeContext => analysisContext.MarkTreeAsIgnoredIfAutoGenerated( treeContext.Tree ) );
		...
		private class UnusedSymbolAnalysisContext
		{
			...
			private readonly HashSet<SyntaxTree> _autoGeneratedTrees = new HashSet<SyntaxTree>();
			...
			private void ReportDiagnostics( IEnumerable<ISymbol> unusedSymbols, Action<Diagnostic> reportDiagnosticAction )
			{
				foreach ( ISymbol unusedSymbol in unusedSymbols )
				{
					...
					foreach ( SyntaxReference declarationReference in definition.DeclaringSyntaxReferences.Where( r => !_autoGeneratedTrees.Contains( r.SyntaxTree ) ) )
					{
			...
			public void MarkTreeAsIgnoredIfAutoGenerated( SyntaxTree tree )
			{
				if ( tree.GetRoot().IsAutoGenerated() )
					_autoGeneratedTrees.Add( tree );
			}
